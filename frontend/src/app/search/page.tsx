...
export default function SearchPage() {
  ...
  const [threads, setThreads] = useState<ChatThreadOut[]>([]);
  const [showThreads, setShowThreads] = useState(false);

  useState(() => { getChatThreads().then(setThreads).catch(() => {}); });
  ...
  const handleChat = async (e: FormEvent) => {
    ...
    setChatLoading(true);
    try {
      const res = await aiChatWithThread(question, threadId);
  ...
  return (
    <div className="w-full min-w-0">
      <div className="mb-6 sm:mb-8 text-center px-1">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Ask Knowledgia</h1>
        <p className="mt-1 text-gray-500 dark:text-gray-400">Search the knowledge base or ask questions in plain language</p>
        {threads.length > 0 && (
          <button onClick={() => setShowThreads(!showThreads)} className="mt-2 text-xs text-brand-600 hover:underline">
            {showThreads ? "Hide" : "Show"} chat history ({threads.length})
          </button>
        )}
      </div>
      ...
      {showThreads && threads.length > 0 && (
        <div className="mb-4 mx-auto max-w-2xl">
          <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 space-y-1 max-h-48 overflow-y-auto">
            {threads.map((t) => (
              <div key={t.id} className="flex items-center justify-between rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 px-3 py-2 transition-colors">
                <button onClick={async () => {
                  const detail = await getChatThread(t.id);
                  setThreadId(t.id);
                ...
                }} className="text-sm text-gray-700 dark:text-gray-200 truncate text-left flex-1">
                  {t.title || "Untitled chat"}
                </button>
                <button onClick={async () => { await deleteChatThread(t.id); getChatThreads().then(setThreads).catch(() => {}); }}
                  className="text-xs text-gray-400 hover:text-red-500 ml-2 shrink-0">&times;</button>
              </div>
            ))}
          </div>
          <button onClick={() => { setThreadId(undefined); setChatMessages([]); setShowThreads(false); }}
            className="mt-2 text-xs text-brand-600 hover:underline">+ New conversation</button>
        </div>
      )}
      ...
      <form onSubmit={handleSearch} className="mb-6 sm:mb-8 mx-auto max-w-2xl w-full px-1">
        <div className="relative">
          ...
          <input type="text" value={query} onChange={(e) => setQuery(e.target.value)}
            ...
            className="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-3 sm:py-4 pl-10 sm:pl-12 pr-20 sm:pr-24 text-sm sm:text-base dark:text-white shadow-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-400/20 outline-none" />
        ...
        <div className="mt-2 flex flex-wrap gap-2 justify-center">
          {["transfer pricing", "VAT reconciliation", "project risk management", "DCF valuation"].map((hint) => (
            <button key={hint} type="button" onClick={() => { setQuery(hint); }}
              className="rounded-full border border-gray-200 dark:border-gray-700 px-3 py-1 text-xs text-gray-500 dark:text-gray-400 hover:border-purple-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
              {hint}
            </button>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-2xl w-full mb-6 sm:mb-8 px-1">
        <div className="rounded-xl border border-purple-200/60 dark:border-purple-800/40 bg-purple-50/30 dark:bg-purple-900/20 p-4">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-purple-500 mb-3 flex items-center gap-1">
            ...
            Ask Knowledgia AI · answers from your REX sheets
          </p>
            ...
            <div className="space-y-3 mb-3 max-h-64 overflow-y-auto">
              {chatMessages.map((msg, i) => (
                <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : ""}`}>
                  ...
                  <div className={`max-w-[85%] rounded-xl px-3 py-2.5 text-sm ${
                    msg.role === "user" ? "bg-brand-600 text-white" : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 shadow-sm"
                  }`}>
              ...
              {chatLoading && (
                <div className="flex gap-2">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-purple-200 text-[10px] font-bold text-purple-700">AI</div>
                  <div className="rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-3 py-2 text-sm text-gray-400">Thinking...</div>
                </div>
              )}
            </div>
          )}
          <form onSubmit={handleChat} className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-sm dark:text-white focus:border-purple-400 outline-none placeholder:text-gray-400"
            />
                ...
                {experts.slice(0, 6).map((e) => (
                  <button key={e.id} onClick={() => setProfileUserId(e.id)} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 hover:shadow-md transition-shadow text-left w-full">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{e.full_name}</p>
